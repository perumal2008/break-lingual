...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    else { setError('Login failed. Please try again.'); setIsLoading(false); }
  };
  ...
  const handleSocialLogin = async (provider) => {
    ...
    if (success) navigate(from, { replace: true });
    else { setError('Social login failed. Please try again.'); setIsLoading(false); }
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full bg-white/5 border border-white/10 text-white placeholder-slate-500 rounded-lg px-4 py-2.5 focus:outline-none focus:border-blue-500 transition"
                required
              />
            </div>
            <div>
